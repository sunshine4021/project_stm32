...
_Bool Info_Read(void)
{
	...
	AT24C02_ReadBytes(DEVID_ADDRESS + 1, (unsigned char *)onenet_info.dev_id, Info_WifiLen(1));		//获取devid长度  读devid
    DelayXms(10);																					//延时
                
    memset(onenet_info.api_key, 0, sizeof(onenet_info.api_key));										//清除之前的内容
	AT24C02_ReadBytes(AKEY_ADDRESS + 1, (unsigned char *)onenet_info.api_key, Info_WifiLen(2));		//获取apikey长度  读apikey
    DelayXms(10);																					//延时
	
	memset(onenet_info.pro_id, 0, sizeof(onenet_info.pro_id));											//清除之前的内容
	AT24C02_ReadBytes(PROID_ADDRESS + 1, (unsigned char *)onenet_info.pro_id, Info_WifiLen(3));		//获取proid长度  读proid
    DelayXms(10);																					//延时
	
	memset(onenet_info.auif, 0, sizeof(onenet_info.auif));											//清除之前的内容
	AT24C02_ReadBytes(AUIF_ADDRESS + 1, (unsigned char *)onenet_info.auif, Info_WifiLen(4));			//获取auif长度  读auif
...
_Bool Info_Alter(char *info)
{
	...
	if((usart1Tmp = strstr(info, "AUIF:")) != (void *)0)								//提取auif
	{
        ...
        if(usart1Count > 0)
        {
			...
			AT24C02_WriteByte(AUIF_ADDRESS, strlen(onenet_info.auif));					//保存auif长度
			RTOS_TimeDly(2);
